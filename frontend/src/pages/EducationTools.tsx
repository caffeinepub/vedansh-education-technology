...
import { Code, Cpu, Wrench, Play, RotateCcw, ChevronRight } from 'lucide-react';
...
public class Main {
    public static void main(String[] args) {
        ...
        System.out.println("Vedansh Education & Technology");
    }
}`,
  'C++': `// C++ Program
#include <iostream>
...
int main() {
    ...
    cout << "Vedansh Education & Technology" << endl;
    return 0;
}`,
...
const algorithmSteps = [
  { step: 1, desc: 'Initialize array with elements', code: 'arr = [64, 34, 25, 12, 22, 11, 90]' },
  { step: 2, desc: 'Compare adjacent elements', code: 'if arr[j] > arr[j+1]: swap' },
  { step: 3, desc: 'Swap if out of order', code: 'arr[j], arr[j+1] = arr[j+1], arr[j]' },
  { step: 4, desc: 'Repeat for all passes', code: 'for i in range(n-1)' },
  { step: 5, desc: 'Array is now sorted!', code: 'Result: [11, 12, 22, 25, 34, 64, 90]' },
];
      ...
      <div className="gradient-hero px-4 pt-6 pb-12">
        <div className="max-w-2xl mx-auto">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <h1 className="font-baloo font-bold text-2xl text-white">Education Tools</h1>
              <p className="text-white/60 text-sm">BTech Branch-wise Interactive Tools</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <Tabs defaultValue="cse">
          ...
          <TabsContent value="cse" className="space-y-4">
            ...
            <div className="bg-card border border-border rounded-2xl overflow-hidden">
              ...
              <div className="p-4 space-y-3">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={() => { setCode(codeTemplates[language] || ''); setOutput(''); }}
                ...
                {output && (
                  <div className="bg-vedansh-navy rounded-xl p-4">
                    <div className="text-vedansh-orange text-xs font-semibold mb-2">Output:</div>
                    <pre className="text-white/80 text-xs font-mono whitespace-pre-wrap">{output}</pre>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="bg-card border border-border rounded-2xl p-5">
              ...
              <div className="bg-muted/50 rounded-xl p-3 mb-4">
                <div className="text-xs text-foreground/60 mb-2 font-medium">Bubble Sort Visualization</div>
                ...
                {algorithmSteps.map((step, i) => (
                  <div
                    ...
                    className={`flex items-center gap-3 p-2 rounded-lg transition-all duration-300 ${
                      algoStep > i ? 'bg-vedansh-success/10 border border-vedansh-success/20' :
                      algoStep === i ? 'bg-vedansh-orange/10 border border-vedansh-orange/20' :
                      'bg-muted/30'
                    }`}
                  >
                    <div className={`w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${
                      algoStep > i ? 'bg-vedansh-success text-white' :
                      algoStep === i ? 'bg-vedansh-orange text-white' :
                      'bg-muted text-foreground/40'
                    }`}>
                      {step.step}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="text-xs font-medium text-foreground">{step.desc}</div>
                      <code className="text-xs text-foreground/50 font-mono">{step.code}</code>
                    </div>
                  </div>
                ))}
              </div>
              ...
              >
                {algoRunning ? <RotateCcw className="w-4 h-4 animate-spin" /> : <Play className="w-4 h-4" />}
          ...
          <TabsContent value="ece" className="space-y-4">
            <div className="bg-card border border-border rounded-2xl p-5">
              ...
              {/* Component Palette */}
              <div className="mb-4">
                <div className="text-xs text-foreground/60 font-medium mb-2">Component Palette</div>
                <div className="grid grid-cols-3 gap-2">
                  {circuitComponents.map((comp, i) => (
                    <button
                      key={i}
                      onClick={() => toast.info(`${comp.name} added to circuit`)}
                    ...
                    >
                      <div className={`font-mono text-lg font-bold ${comp.color} mb-1`}>{comp.symbol}</div>
                      <div className="text-foreground/70 text-xs">{comp.name}</div>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="bg-muted/30 border-2 border-dashed border-border rounded-xl h-40 flex items-center justify-center mb-4 relative overflow-hidden">
                ...
                <svg className="absolute inset-0 w-full h-full opacity-20" viewBox="0 0 300 160">
                  <line x1="20" y1="80" x2="280" y2="80" stroke="#FF6B00" strokeWidth="2" />
                  <line x1="150" y1="20" x2="150" y2="140" stroke="#FF6B00" strokeWidth="2" />
                  <circle cx="150" cy="80" r="8" fill="#FF6B00" />
                </svg>
              </div>

              <button
                onClick={() => toast.info('Circuit simulation running... (UI Demo)')}
            ...
            {/* ECE Topics */}
            <div className="bg-card border border-border rounded-2xl p-5">
              <h3 className="font-semibold text-foreground mb-3">ECE Study Topics</h3>
              <div className="space-y-2">
                {['Digital Electronics', 'Analog Circuits', 'Signals & Systems', 'Microprocessors', 'Communication Systems', 'VLSI Design'].map(topic => (
                  <button
                    key={topic}
          ...
          <TabsContent value="me" className="space-y-4">
            <div className="bg-card border border-border rounded-2xl p-5">
              ...
              <div className="bg-gray-900 rounded-xl h-48 flex items-center justify-center mb-4 relative overflow-hidden">
                ...
                <div className="absolute bottom-2 left-2 text-white/40 text-xs">3D CAD Viewport (Demo)</div>
                <div className="absolute top-2 right-2 flex gap-1">
                  {['X', 'Y', 'Z'].map(axis => (
                    <span key={axis} className="text-xs bg-white/10 text-white/60 px-1.5 py-0.5 rounded">{axis}</span>
                  ))}
                </div>
              </div>
              ...
              {/* Parts List */}
              <div>
                <div className="text-xs text-foreground/60 font-medium mb-2">Parts List</div>
                <div className="space-y-2">
                  {cadParts.map((part, i) => (
                    <div key={i} className="flex items-center gap-3 p-2 bg-muted/50 rounded-lg">
                      ...
                      <div className="flex-1">
                        <div className="text-foreground text-xs font-medium">{part.name}</div>
                        <div className="text-foreground/50 text-xs">{part.material} • {part.dims}</div>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <TabsContent value="others" className="space-y-4">
            {[
              { branch: 'Civil Engineering (CE)', icon: '🏗️', tools: ['AutoCAD Basics', 'Structural Analysis', 'Surveying Tools', 'Concrete Mix Design'] },
              { branch: 'Electrical Engineering (EE)', icon: '🔌', tools: ['Power Systems', 'Electrical Machines', 'Control Systems', 'Power Electronics'] },
              { branch: 'IT Engineering', icon: '🌐', tools: ['Web Development', 'Database Design', 'Network Security', 'Cloud Computing'] },
              { branch: 'Chemical Engineering', icon: '🧪', tools: ['Process Simulation', 'Heat Transfer', 'Mass Transfer', 'Reaction Engineering'] },
            ].map((branch, i) => (
              <div key={i} className="bg-card border border-border rounded-2xl p-5">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="font-semibold text-foreground flex items-center gap-2">
                    <span className="text-xl">{branch.icon}</span> {branch.branch}
                  </h3>
                  <GovtApprovedBadge size="sm" />
                </div>
                <div className="grid grid-cols-2 gap-2">
                  {branch.tools.map(tool => (
                    ...
                    >
                      <div className="text-foreground text-xs font-medium">{tool}</div>
                      <div className="text-foreground/40 text-xs mt-0.5">Interactive Tool</div>
                    </button>
                  ))}
                </div>
              </div>
